...
sensors_poll_context_t::sensors_poll_context_t()
{
    ...
    mSensors[ISL29030L] = new ISL29030SensorL();
    ...
    mSensors[ISL29030P] = new ISL29030SensorP();
...
int sensors_poll_context_t::activate(int handle, int enabled) {
    ...
    if (!err && handle == SENSOR_TYPE_ORIENTATION) {
        err = static_cast<KXTF9Sensor*>(
                mSensors[KXTF9])->enableOrientation(enabled);
    }
    if (enabled && !err) {
